...
```mermaid
flowchart TD
    subgraph Programming["💻 第四层：编程层 (Programming)"]
        P1["Agent SDK<br>(Python / TypeScript)"] --- P2["从使用者<br>转变成构建者"]
    end

    subgraph Integration["🔗 第三层：集成层 (Integration)"]
        I1["Headless模式"] --- I2["CI/CD自动化"] --- I3["MCP"] --- I4["外部工具·数据源"]
    end

    subgraph Extension["🔌 第二层：扩展层 (Extension)"]
        E1["Commands<br>斜杠命令/手动触发"] --- E2["Skills<br>技能/语义触发"] --- E3["SubAgents<br>子智能体/任务委派"] --- E4["Hooks<br>事件钩子/事件触发"]
    end

    subgraph Memory["🧠 第一层：记忆层 (Memory)"]
        M1["Enterprise→User→<br>Project→Rules→Local"] --- M2["CLAUDEM<br>记忆系统"]
    end

    Programming --> Integration --> Extension --> Memory
    ...
    classDef programming fill:#fff3e0,stroke:#e65100
    class Programming,Extension,Integration,Memory memory,extension,integration,programming
```
...
graph TD
    ...
    B["./CLAUDE.md"]
    B_desc["项目级：团队共享的项目约定"]
    B -.-> B_desc

    C[".claude/rules/*.md"]
    C_desc["规则级：模块化的细粒度规则"]
    C -.-> C_desc

    D[".claude.local.md"]
    D_desc["本地级：不提交到 Git 的私人笔记"]
    D -.-> D_desc
...
通过标准协议无缝接入外部系统，轻松扩展Claude的能力边界，是AI时代的 type-c
...
```
claude.md  专注于让Claude知道什么，奠定基础上下文
Skill：专注于让怎么做，。租入领域专业知识
Command:提供标准化的人工操作入口
subagent：专注于让确定谁来做，拆解并处理高复杂度任务
Hooks:专注于能否做？构建自动化的安全防线
MCP：打通外部系统的数据壁垒
Headless:将能力无缝嵌入CI/CD流水线

每个角色各司其职，边界清晰、紧密协同达成最终目标。
```



技术选型的核心原则始终是：用最简单的方案解决当前问题，切勿陷入手里有锤子，看什么都是钉子的误区。过度工程化与工程化不足同样有害。
