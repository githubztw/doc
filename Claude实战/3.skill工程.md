...
[技能创建者的最佳实践——特工技能](https://agentskills.io/skill-creation/best-practices)
...
最有用的“应该触发”查询是那些技能会有帮助但仅凭查询联系不明显的

**不应该触发的测试用例：**

最有价值的负面测试案例是——那些与你技能共享关键词或概念，但实际上需要不同内容的查询
...
- **成功的步骤**——导致成功的一系列行动
- **你做的修正**——你引导agent采取方法的地方（例如，“用库X代替Y”、“检查边缘情况Z”）
- **输入/输出格式**
...
- 内部文档、运行手册和风格指南
- API 规范、模式和配置文件
- 代码审查评论和问题跟踪器（记录反复出现的关切和审核员期望）
- 版本控制历史，尤其是补丁和修复（通过实际变化揭示模式）
- 现实世界的失败案例及其解决方案
...
3. 简明、分步骤的指导并附有可行的示例，往往比详尽的文档更有效。当你发现自己要处理所有边缘案件时，考虑大多数情况是否更适合由`subagent/agent`自行判断。
...
1. 任务允许多样实现方式、对执行细节不敏感，指令应当宽松。相比死板的硬性规定，向智能体**解释背后的目的**效果更好。
   ...
   ```
   ## 代码审查流程
   1. 检查所有数据库查询，防范SQL注入（需使用参数化查询）
   2. 校验每个接口端点的身份鉴权逻辑
   3. 查找并发代码路径下的竞态条件问题
   4. 确认错误日志不会泄露系统内部敏感信息
   ```
...
1. **场景专属避坑清单**：记录那些违背常规直觉、容易踩坑的环境特殊规则，精准修正智能体在无人提醒时极易犯下的具体错误。
   ...
   ```
   ## 避坑清单
   - `users` 用户表采用软删除机制，所有查询必须带上条件 `WHERE deleted_at IS NULL`，否则会查出已注销停用的账号。
   - 用户标识在数据库内字段为 `user_id`、鉴权服务中为 `uid`、账单API内为 `accountId`，三者指向同一用户主体。
   - `/health` 健康接口仅保证Web服务进程存活，即便数据库断开连接也会返回200状态码；如需完整服务健康检测，请调用 `/ready` 接口。
   ```
...
5. **规划 - 校验 - 执行**
   ...
   ```
   ## PDF表单填充流程
   1. 提取表单字段：`python scripts/analyze_form.py input.pdf` → 输出`form_fields.json`
   （该文件会列出全部字段名、字段类型、是否为必填项）
   2. 创建`field_values.json`，建立字段与待填充值的映射关系
   3. 执行校验：`python scripts/validate_fields.py form_fields.json field_values.json`
   （校验规则：所有填写字段必须存在于表单定义、字段类型匹配、必填项无缺失）
   4. 若校验失败，修改`field_values.json`并重新校验
